...
class a_decoder(Layer):
    ...
    def compute_query(self, last_3_actions):
        """Computes query vector based on the last 3 cities visited."""
        que = tf.nn.relu(self.W1(last_3_actions[:, -1, :])+
                         self.W2(last_3_actions[:, -2, :])+
                         self.W3(last_3_actions[:, -3, :]))
        return que
    def call(self, action_vectors):
        batch_size,num_elements, dmodel = action_vectors.shape
        ...
        for _ in range(num_elements):
            ...
            mask = tf.tensor_scatter_nd_update(mask, tf.expand_dims(next_index, axis=-1), tf.cast(tf.ones_like(next_index), tf.bool))
            selected_indices.append(next_index)
            next_action = tf.gather(action_vectors, next_index, batch_dims=1)
            next_action = tf.gather(next_action, axis=1)
            last_actions = tf.concat([last_actions[:,1:,:], tf.expand_dims(next_action, axis=-1)], axis=1)
            query_state = self.compute_query(last_actions)
        return tf.stack(selected_indices, axis=1), probability
